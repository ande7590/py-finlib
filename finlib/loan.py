...
class LoanAmortization(object):    
    """Amortizes a loan.  Specify loan parameters in constructor. Generate
     ...
     + pmt_beg_per - Annuity immediate (False) or annuity due (True)
     """

    def __init__(self, term: int, interest: float, loan_amt: float, pmt_beg_per:bool=False):
        self.__mutable = True
        self.term = term 
        self.interest = interest
        self.loan_amt = loan_amt
        self.pmt_beg_per = pmt_beg_per
    ...
    def amortize(self, current_balance: float=None, pmts: Iterable=None)\
        ...
        # and solve for the number of remaining payments
            ...
            i = self.interest
            v = pow(1+i, -1)
            int_fctr = i/(1+i) if self.pmt_beg_per else i
